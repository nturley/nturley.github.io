...
function wirePull(pullX, pullY, offset, alpha)
{
	return function(node){
		...
		node.inputPorts.forEach(function(i)
		{
			...
			var myX = i.x + i.daddy.x;
			var myY = i.y + i.daddy.y;
			
			var driverX = i.wire.driver.x + i.wire.driver.daddy.x;
			var driverY = i.wire.driver.y + i.wire.driver.daddy.y;

			var x = driverX-myX-offset;
			var y = driverY-myY;
			if (x>0)
			{
				node.x += x*pullX*alpha;
			} else {
				node.x += x*pullX*alpha;
			}
			node.y += y*pullY*alpha;
			
		});
		node.outputPorts.forEach(function(o)
		{
			var myX = o.x + o.daddy.x;
			var myY = o.y + o.daddy.y;
			o.wire.riders.forEach(function(r)
			{
				if (r.value.feedback == true) {
					return;
				}
				var rx = r.value.x + r.value.daddy.x;
				var ry = r.value.y + r.value.daddy.y;
